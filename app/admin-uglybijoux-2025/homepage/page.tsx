...
export default function HomepageProduct() {
  const { data: shop, mutate: refreshShop } = useSWR('http://localhost:5000/products/shop', fetcher);
  const { data: homepage, mutate: refreshHomepage } = useSWR('http://localhost:5000/products/homepage', fetcher);
  ...
  const toggleHomepage = async (productId: string, currentlyShown: boolean) => {
    ...
    const url = `http://localhost:5000/products/homepage/${productId}`;
